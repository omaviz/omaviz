//! PipeWire capture: taps the default sink monitor and mixes to mono f32.
...
pub fn run(tx: Sender<AudioChunk>) -> Result<()> {
    pw::init();
    ...
    let _listener = stream
        ...
        .param_changed(|_, ud, id, param| {
            ...
            if ud.format.parse(param).is_ok() {
                eprintln!(
                    "omaviz: capturing rate:{} channels:{}",
                    ud.format.rate(),
                    ud.format.channels()
                );
            }
