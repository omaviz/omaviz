...
// ALL scalars ride as exact texels in control row 3 (NEAREST sampling):
// x0 visual/spikes/splits/fire, x1 dots/reflect/art/mono,
// x2 monolight/sync/thickness/peaks, x3 themeBottom.rgb, x4 themeTop.rgb,
// x5 barCount, x6 gapPx, x7 width/height hi+lo bytes.
...
float u_nb() { return floor(cell(5.0).r * 512.0 + 0.5); }
float u_gap() { return floor(cell(6.0).r * 8.0 + 0.5); }
...
float u_thick() { return cell(2.0).b * 5.0; }
...
float waveAt(float i)  { return texture(u_tex, vec2((clamp(i, 0.0, 127.0) + 0.5) / TEXW, 2.5 / 4.0)).r * 2.0 - 1.0; }
