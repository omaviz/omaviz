...
| 29 | **T-028 bar-widget realignment (ADR-0014):** root cause confirmed against `WidgetButton.qml`: `text:""` → base `hasVisualContent=false` → opacity 0 + width ~29px (spectrum invisible). Dev's `4e07a74` widened `implicitWidth` but left `text:""`, so **NOT fixed** (lotus's capture proves empty bar; third false-green claim). Required: non-empty `text` (e.g. `" "`) OR `fixedWidth`, or render spectrum as unicode-block `text` (lotus's approach). Plus Panel base wrap + drop Detach UI (those parts done). | In Progress | 0% | (spec: ADR-0014) |
