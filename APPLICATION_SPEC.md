...
| 20 | Settings-panel popup invisible (T-014): **Accepted, scoped to detached/standalone path.** Verified (lotus 48-log re-grep: 8 standalone/0 shell) the `qs.Commons`/`qs.Ui` unresolvable-import warning occurs only when Panel.qml/BarWidget load standalone (detach path) — the mini bar/shell resolves qs.* fine (genuine shell defect is T-019 `anchors.fill`). Engine resolves `qs.Commons`→`plugin/Commons` but vendored copy sits at `plugin/qs/Commons` (one level too deep) → warning. Fix: path correction (`plugin/qs/Commons`→`plugin/Commons`) or add to import path. | Backlog | 0% | (spec: ADR-0008) |
