...
| 23 | Detached desktop window never maps (T-018): root `Window` resolves to `QtQuick.Window` (plain, untracked) not Quickshell `QsWindow` → not surfaced by Hyprland. Fix: use Quickshell `Window` (drop `import QtQuick` shadowing / qualify). CONFIRMED from production pid 9927/9948 log. | Backlog | 0% | (spec: ADR-0012) |
