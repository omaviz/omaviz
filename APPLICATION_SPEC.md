...
| 20 | Settings-panel popup invisible (T-014): **CONFIRMED root cause** = `import qs.Commons`/`qs.Ui` unresolvable in BOTH standalone + shell-driven launches (31-log grep) → Panel.qml fails to load. Fix: vendor minimal qs.Commons + used qs.Ui into plugin/qs/{Commons,Ui} with qmldir. Anchor fallback (85cde08) kept as necessary-but-insufficient; T-019 (ADR-0011) compounds. | Backlog | 0% | (spec: ADR-0008) |
