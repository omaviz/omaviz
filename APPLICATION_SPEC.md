...
| 20 | **T-014 settings panel never surfaces** — confirmed live QML error `Panel.qml:120: Cannot assign to non-existent property "fill"` (`KeyboardPanel`) aborts Panel construct. `qs.*` resolves in shell (mini works) ⇒ not a missing-module issue. Fix in ADR-0008. | In Progress | 0% | (spec: ADR-0008) |
| 21 | **T-015 detached window maps but blank/unresponsive** — leading hypothesis: 2nd engine PipeWire node-name collision (ADR-0009); verify on live shell. T-018 "never maps" reclassified as layer-shell measurement artifact (user sees the window). | In Progress | 0% | (spec: ADR-0009) |
| 22 | Click mapping left vs right (T-016): **BLOCKED pending user decision** — current mapping left=settings/right=detach stands as default. ADR-0010. | In Progress | 0% | (spec: ADR-0010) |
| 23 | **UI integration test gate (ADR-0006):** unit suites insufficient — require panel-popup visibility + detached-window bands-flow + click-mapping integration tests. Closes "green gates, broken UI" gap. | In Progress | 0% | (spec: ADR-0006) |
| 24 | **T-019** `Panel.qml:120` QML error `Cannot assign to non-existent property "fill"` — real error halting settings popup; valid geometry on `KeyboardPanel` (folded into ADR-0008). | In Progress | 0% | (spec: ADR-0008) |
