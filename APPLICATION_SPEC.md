...
| 20 | Settings-panel popup invisible (T-014): **Accepted, scoped to detached/standalone path.** Verified by lotus via by-id log grep: every detached launch emits `Ignoring unresolvable import .../Commons|Ui` from BOTH BarWidget.qml AND Panel.qml. Architect runtime test (`quickshell -p plugin/Panel.qml`) reproduces it standalone. Mini bar/in-shell resolves qs.* (shell supplies import path); genuine shell defect is T-019 `anchors.fill`. Engine resolves `qs.Commons`→`plugin/Commons` but vendored copy sits at `plugin/qs/Commons` (one level too deep) → warning. Fix: path correction (`plugin/qs/Commons`→`plugin/Commons`) or add to import path. | Backlog | 0% | (spec: ADR-0008) |
...
| 23 | Detached desktop window never maps (T-018): **CLOSED — layer-shell artifact.** hyprctl clients is also blind to the omarchy bar, so "no mapped omaviz window" was a measurement blind spot, not a defect (empirical: bare/real/child Desktop.qml all mapped under hyprctl). No code fix; T-018 fix dropped per lotus. (Original "import QtQuick shadows Window" theory RETRACTED as false.) | Closed | 100% | (spec: ADR-0012) |
...
| 29 | **T-028 bar-widget realignment (ADR-0014):** root cause confirmed against `WidgetButton.qml`: `text:""` → base `hasVisualContent=false` → opacity 0 + width ~29px (spectrum invisible). **FIXED & independently verified 2026-08-21**: deployed fix uses non-empty spectrum `text` (unicode blocks, ADR-0014 §2.1 (c)); independent vision of `/tmp/bar_top_small.png` confirms the omaviz spectrum now renders in the bar's right section (varying-height white bars). Dev's `4e07a74` "verified working" was false (left `text:""`); the later unicode-text deploy (lotus's approach) is the real fix. Panel base wrap + Detach-UI removal also done. **Last open item:** panel-open frame after @user's one left-click (needs capture). | In Progress | 90% | (spec: ADR-0014) |
