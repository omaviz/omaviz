...
| 28 | **T-025 capture-toolchain broken (QA blocker, NOT omaviz):** `grim` hard-hangs (rc=124) on full + single-output; `hyprctl screenshot` = "unknown request" on this build; no `wlrctl`/`wf-recorder`/`ydotool`. Blocks mini-bar render + panel-open proof. Independent of omaviz code. | Open | 0% | (no ADR — harness/infra) |
