...
| 20 | Settings-panel popup invisible (T-014): **HELD — import-resolution UNVERIFIED**. Runtime test: engine resolves `qs.Commons`→`plugin/Commons` (vendored copy is misplaced at `plugin/qs/Commons`, one level too deep) → "Ignoring unresolvable import" warning, BUT config still "Configuration Loaded" (not a hard fail). No 31-log grep exists; qmllint doesn't confirm. Needs controlled runtime load on user's live system (GPU/UI approval) before any module fix. `anchors.fill` (T-019/ADR-0011) is the code-verified companion cause, routed separately. | Backlog | 0% | (spec: ADR-0008) |
...
| 23 | Detached desktop window never maps (T-018): **BLOCKED — root cause UNCONFIRMED; retracted "import QtQuick shadows Window" theory (false: QtQuick has no Window type; bare/real/child Desktop.qml all MAPPED in this env). Need live repro on user's system: capture hyprctl clients + quickshell log + env diff. Do NOT change window type yet. | Backlog | 0% | (spec: ADR-0012) |
